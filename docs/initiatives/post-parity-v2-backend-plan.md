...
| B23 | Run thumbnails — B17 wire extension + presigned-GET + sidecar PNG capability | T2 | Cross-repo with desktop SP1 (`pv_layout_project` PLAN.md Phase 6). Source: SMOKE-LOG.md S1-06 (Session 1, 2026-04-30 — run gallery cards render empty thumbnail placeholders). Likely scope (refined per SP1 design memo at `pv_layout_project/docs/post-parity/findings/2026-04-30-001-run-thumbnail-pipeline.md`): (a) Schema — `Run.thumbnailBlobUrl String?` migration, nullable so pre-pipeline runs render the placeholder cleanly (no backfill data move). (b) Wire — extend `RunDetailWire` in `packages/shared/src/types/project-v2.ts` with `thumbnailBlobUrl: string \| null`; B17's `getRunDetail` mints a presigned-GET via existing `getPresignedDownloadUrl` (mirror the `layoutResultBlobUrl` pattern, same 1h TTL). (c) Storage — extend `RUN_RESULT_SPEC` in `blobs.service.ts` with a `thumbnail` type so B7 can mint upload URLs for it; key path `projects/<userId>/<projectId>/runs/<runId>/thumbnail.png` (or `.webp` per memo). (d) Sidecar contract — extend `/layout` (or new `/layout-thumbnail`) to return a PNG alongside the layout JSON; desktop PUTs post-Generate via B7. Open sub-decisions for the memo: PNG vs WebP, dimensions, render-on-Generate vs on-demand, cost (~$0.30/mo for 100k runs at ~10KB each in `ap-south-1`). | B17, SP1 design memo | B17 returns `thumbnailBlobUrl` field (null for pre-pipeline runs, presigned URL otherwise). Desktop renders the thumbnail when present, falls back to placeholder on null. Backfill validation: existing fixture runs return null cleanly. Per-run cost stays under $0.005/yr at projected volume. Integration test for the new wire field; sidecar PNG-render unit test. | todo — awaiting SP1 design memo |
