...
**Clean-build gate (mandatory when workspace package resolution changes):**

Run this instead of the standard gate whenever any of the following change:
- `turbo.json` task dependencies (`dependsOn`)
- `tsconfig.json` `paths` aliases pointing to workspace packages
- `next.config.*` `transpilePackages`
- A new workspace package is added or a package's `package.json` `exports`/`main` changes
...
**Why**: Local dev accumulates stale `dist/` artifacts from prior builds. The standard gate re-uses them, masking resolution errors that only surface on CI (clean environment, no cache). The clean-build gate replicates CI conditions exactly.

**Incident record — 2026-04-19**: Added `@renewable-energy/api-client` as a new workspace package using NodeNext module resolution (`.js` extensions in source imports). Initially wired a tsconfig path alias pointing to source — local gates passed because Turbopack found an existing `dist/` from a previous build and never followed the alias into source. CI failed (`Can't resolve './client.js'`) because no `dist/` existed. Fix: removed the path alias, removed `transpilePackages`, added `@renewable-energy/web#typecheck: { dependsOn: ["^build"] }` to `turbo.json` so `api-client#build` runs first. Clean-build gate would have caught this immediately.
...
- **Lucide React** (`lucide-react`) — not Phosphor; import as `import { ChevronDown } from "lucide-react"`
...
- **UI package moduleResolution**: `packages/ui/tsconfig.json` uses `"Bundler"` (not `"NodeNext"`)
- **shadcn combobox omitted**: nova style ships a combobox using `@base-ui/react` which crashes Turbopack SSR (`createContext` failure); do not add it — use Command-based combobox instead

## Workspace Package Resolution

Two resolution strategies exist. Choose based on the package's module format:

| Strategy | When to use | How |
|---|---|---|
| **Source alias** | Pure-type packages or packages using `"Bundler"` moduleResolution (no `.js` extension imports) | Add to `tsconfig.json` `paths` and `transpilePackages` |
| **Compiled dist** | Packages using `"NodeNext"` moduleResolution (`.js` extension imports in source) | Do NOT alias to source; add `@renewable-energy/<pkg>#typecheck: { dependsOn: ["^build"] }` and `@renewable-energy/<pkg>#build: { dependsOn: ["^build"] }` to `turbo.json` |

Current assignments:
- `@renewable-energy/ui` → source alias + `transpilePackages` (`"Bundler"` resolution)
- `@renewable-energy/shared` → source alias in `tsconfig.json` (`"Bundler"` resolution)
- `@renewable-energy/api-client` → compiled dist via turbo pipeline (`"NodeNext"` resolution)
- `@renewable-energy/db` → compiled dist via turbo pipeline (`"NodeNext"` resolution)

**Rule**: When adding a new package, identify its `moduleResolution` setting first. If `NodeNext`, it must use the compiled dist strategy. Never point a Next.js/Turbopack path alias at NodeNext source — Turbopack cannot remap `.js` imports to `.ts` files.
